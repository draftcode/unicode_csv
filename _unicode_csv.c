...
module.  Users should not use this module directly, but import the unicode_csv.py
module instead.
...
typedef struct {
    ...
    Py_UNICODE *field;             /* temporary buffer */
    ...
    if (src == NULL)
        ...
        *target = '\0';
        if (src != Py_None) {
            ...
            if (len > 0)
                *target = PyUnicode_AS_UNICODE(src)[0];
        }
    }
    return 0;
}
...
Reader_iternext(ReaderObj *self)
{
    ...
    do {
        ...
        line = PyUnicode_AS_UNICODE(lineobj);
 ...
 */
static Py_ssize_t
join_append_data(WriterObj *self, Py_UNICODE *field, Py_ssize_t field_len,
                 int quote_empty, int *quoted, int copy_phase)
{
    DialectObj *dialect = self->dialect;
    Py_ssize_t i, j;
    Py_ssize_t rec_len, lineterm_len;
    Py_UNICODE *lineterm;
...
#define ADDCH(c) \
    ...
    if (lineterm == NULL)
        return -1;
    lineterm_len = PyUnicode_GetSize(dialect->lineterminator);
...
static int
join_append(WriterObj *self, Py_UNICODE *field, Py_ssize_t field_len, int *quoted, int quote_empty)
{
    Py_ssize_t rec_len;

    rec_len = join_append_data(self, field, field_len, quote_empty, quoted, 0);
    if (rec_len < 0)
        return 0;
    ...
    self->rec_len = join_append_data(self, field, field_len, quote_empty, quoted, 1);
    self->num_fields++;
...
join_append_lineterminator(WriterObj *self)
{
    ...
    for (i = 0; i < terminator_len; i++)
        self->rec[self->rec_len + i] = terminator[i];
...
csv_writerow(WriterObj *self, PyObject *seq)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        if (PyUnicode_Check(field)) {
            append_ok = join_append(self,
                                    ...
                                    PyUnicode_GetSize(field),
                                    &quoted, len == 1);
            Py_DECREF(field);
        }
        else if (field == Py_None) {
            append_ok = join_append(self, NULL, 0, &quoted, len == 1);
            Py_DECREF(field);
        }
        else {
            PyObject *str;
            ...
            if (str == NULL)
                return NULL;
            append_ok = join_append(self, PyUnicode_AS_UNICODE(str),
                                    PyUnicode_GetSize(field),
                                    &quoted, len == 1);
            Py_DECREF(str);
        }
    ...
    line = PyUnicode_FromUnicode(self->rec, self->rec_len);
    if (line == NULL)
        return NULL;
...
"\n"
"        unicode_csv.QUOTE_MINIMAL means only when required, for example, when a\n"
"            field contains either the quotechar or the delimiter\n"
"        unicode_csv.QUOTE_ALL means that quotes are always placed around fields.\n"
"        unicode_csv.QUOTE_NONNUMERIC means that quotes are always placed around\n"
"            fields which do not parse as integers or floating point\n"
"            numbers.\n"
"        unicode_csv.QUOTE_NONE means that quotes are never placed around fields.\n"
...
"    csv_writer = unicode_csv.writer(fileobj [, dialect='excel']\n"
"                                    [optional keyword args])\n"
"    for row in sequence:\n"
...
"    names = unicode_csv.list_dialects()");

PyDoc_STRVAR(csv_get_dialect_doc,
"Return the dialect instance associated with name.\n"
"    dialect = unicode_csv.get_dialect(name)");

PyDoc_STRVAR(csv_register_dialect_doc,
"Create a mapping from a string name to a dialect class.\n"
"    dialect = unicode_csv.register_dialect(name, dialect)");

PyDoc_STRVAR(csv_unregister_dialect_doc,
"Delete the name/dialect mapping associated with a string name.\n"
"    unicode_csv.unregister_dialect(name)");

PyDoc_STRVAR(csv_field_size_limit_doc,
"Sets an upper limit on parsed fields.\n"
"    unicode_csv.field_size_limit([limit])\n"
"\n"
...
init_unicode_csv(void)
{
    ...
    module = Py_InitModule3("_unicode_csv", csv_methods, csv_module_doc);
    if (module == NULL)
        return;
    ...
    error_obj = PyErr_NewException("_unicode_csv.Error", NULL, NULL);
    if (error_obj == NULL)
        return;
